...
class TestProducers(unittest.TestCase):
    ...
    def test_producer(self, fake_connect, fake_write):
        ...
        fake_socket = fake_connect.expects_call().with_args(
            ('localhost', 1234)).returns_fake()
        fake_write.expects_call().with_args(
            expected_msg).returns(len(expected_msg))
    ...
    def test_batch_producer(self, fake_connect, fake_write):
        ...
        fake_socket = fake_connect.expects_call().with_args(
            ('localhost', 1234)).returns_fake()
        fake_write.expects_call().with_args(
            expected_msg_one).returns(
            len(expected_msg_one)).next_call().with_args(
                expected_msg_two).returns(len(expected_msg_two))
        p = producer.BatchProducer(
            'FakeTopic', batch_interval, host='localhost', port=1234)
